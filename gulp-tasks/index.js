...
/* Configuration */
const { CSS, IMG, HTML, ICON, JS, MJS, PATH } = require('./config.json');
const production = require('./helper/mode');

/* Build */
const buildTask = series(
  clean,
  parallel(copy, css, font, icon, image, js, html, mjs),
);
...
const devTask = series(buildTask, serve, () => {
  ...
  // html
  watch(
    [PATH.src + HTML.src, PATH.src + HTML.basePath + '/*.html'],
    series(html, browserReload),
  );
  // icon
